...
var STATUS_ORDER = ["record", "lookup", "inline", "enum", "string", "derived"];
var ENTITIES = [
  {
    key: "band",
    label: "Band",
    ...
    blurb: "One vibrational feature of one species, with the wavenumber window it is reported in. The hub of the whole dataset.",
    fields: [
      ...
      { name: "wn_start / wn_end", type: "int", req: "req", note: "The band\u2019s window in cm\u207B\xB9. wn_min/max/center derive from it." },
    ...
    open: "intensity, width and confidence describe an observation but are authored once per band. They belong on the assignment, with the band keeping a rollup for the chart."
  },
  {
    key: "assignment",
    label: "Assignment",
    ...
    blurb: "One paper\u2019s claim about one band: the wavenumber it reports, the surface it saw it on, how it was measured, the caveat it attaches. The join between Band and Reference, and the home of everything observation-dependent.",
    fields: [
    ...
    open: "Reaction conditions (temperature, pressure, feed, pretreatment) are still prose inside note. Structuring them only pays off with several times the current number of claims."
  },
  {
    key: "reference",
    label: "Reference",
  ...
  },
  {
    ...
    blurb: "What the band\u2019s atoms are doing: the motion, its symmetry, and which rotational branch of it this band is. Three closed enums in one object, validated in schema.py, and the only descriptor on a band that is fully vocabulary-controlled.",
    fields: [
  ...
  },
  {
    ...
    blurb: 'Which atoms actually move: the bond environment, written as a small formula (O=C=O, C-H, M-O, M-O-C), with "diverse" for a combination whose parents involve unrelated groups. Drives the atoms colormap and the chip on the vibration-modes page.',
    fields: [
  ...
  },
  {
    ...
    id: "key",
    blurb: "The chemical identity a band belongs to, and nothing else. The old free-text label carried five facts at once (identity, phase, binding geometry, isotopologue, sometimes the site); each of those now has its own field.",
    fields: [
  ...
  },
  {
    ...
    id: "key",
    blurb: "Where a band was measured, at whatever scale the paper stated. One table rather than a Site table and a Material table, because the same entry genuinely plays both roles: TiO\u2082 is the sample when a paper measures bare titania and a constituent when that titania supports Pt. `level` records how specific the entry is (site, phase, sample), and the badge is drawn from it: a site is filled, a phase or sample hollow.",
    fields: [
    ...
    open: "A phase\u2019s parts are mostly empty: TiO\u2082 really means a Ti\u2074\u207A or a surface O, but those site records are only worth adding when a paper actually distinguishes them."
  },
  {
    key: "technique",
    label: "Technique",
    ...
    id: "the enum value",
    blurb: "How the spectrum was taken. Used to be three tags mixed in with evidence and caveat tags; it is a field on the assignment now, with a closed vocabulary.",
    fields: [
  ...
  },
  {
    ...
    id: "key",
    blurb: "A named selection of groups, offered by the band chart\u2019s filter. Which families belong to one question is an editorial judgement about the dataset, the same kind of statement as a group\u2019s colour, so it lives with the data rather than in the frontend.",
    fields: [
    ...
    open: '"All groups" is built into the filter rather than authored here, and a selection that matches no set shows as "Custom". Neither is a record.'
  },
  {
    key: "lane",
    label: "Lane",
    ...
    blurb: "One row of the chart, naming the groups that share it. Two groups share a row when they read as one family on screen: CO with CH\u2084, methoxy with the methanol it turns into.",
    fields: [
    ...
    open: "This replaced a per-band `pair` integer that said the same thing 92 times over and left the row order falling out of the numbering rather than being written down."
  },
  {
    key: "region",
    label: "Region",
    ...
    id: "key",
    blurb: "A named stretch of the spectrum. A band is not assigned one: Band.region_for() picks whichever region contains the band centre, so the two can never disagree. The pattern the rest of the model copies.",
    fields: [
  ...
  },
  {
    ...
    id: "the tag string",
    blurb: "A label attached to a band, an assignment or a vibration mode. tags.jsonc is a tooltip lookup only: it does not gate which tags are allowed, so the namespace stays open.",
    fields: [
  ...
  },
  {
    ...
    blurb: "One binding geometry of a molecule: monodentate, bidentate, linear, bridged, and so on. Carries its own point group, because geometry changes it. Now does double duty as the vocabulary for band.topology.",
    fields: [
  ...
  },
  {
    ...
    id: "id",
    blurb: "One named normal mode. Bands point up at modes rather than modes listing bands, so the link is authored in exactly one place; category, atoms and often subtype are then derived from the linked bands.",
    fields: [
      { name: "id / label", type: "string", req: "req", note: "Identity and display name." },
...
var ENTITY_BY_KEY = Object.fromEntries(
...
var TAG_TARGETS = ["band", "assignment", "mode"];
var SELF_LINKS = [
  {
    key: "based_on",
    label: "Built from",
...
function isUmbrellaTag(tag, role) {
  return tag === role;
}
function tagRoleRank(role) {
...
var SURFACE_LEVELS = ["site", "phase", "sample"];
var LEVEL_LABEL = {
  site: "Site",
  phase: "Phase",
  sample: "Sample"
...
var SITE_KIND_LABEL = {
  ...
  defect: "Defect",
  interface: "Interface",
  bronsted: "Br\xF8nsted site"
};
var SITE_KIND_NOTE = {
  metal: "A metal atom in the zero oxidation state, e.g. Cu\u2070.",
  cation: "A Lewis-acidic cation at a stated oxidation state, e.g. Zn\xB2\u207A.",
...
var byUses = (a, b) => b.uses - a.uses || a.value.localeCompare(b.value);
function authorNames(raw) {
  return raw.split(/\s+and\s+/i).map((a) => a.replace(/[{}]/g, "").trim()).filter(Boolean);
}
function analyse(dataset, refs, vibrations, tagTips) {
  const bands = dataset.bands;
  const speciesTable = dataset.species ?? {};
  const surfaceTable = dataset.surfaces ?? {};
  const partsBelow = (key) => {
  ...
  const tagScope = (value, scope, band) => {
  ...
  let modes = 0;
  let topologies = 0;
  const topologyIds = /* @__PURE__ */ new Set();
  ...
  for (const [key, entry] of Object.entries(refs ?? {})) {
    for (const name of authorNames(entry["author"] ?? "")) touch(authors, name, void 0, key);
  }
  ...
  for (const [key, row] of surfaces) {
    ...
    row.level = rec?.level;
    row.kind = rec?.kind ?? void 0;
    row.parts = rec?.parts ?? [];
    row.detail = rec ? rec.level === "site" ? rec.element ? `${rec.element}${rec.oxidation_state != null ? ` (${rec.oxidation_state >= 0 ? "+" : ""}${rec.oxidation_state})` : ""}` : "" : [rec.composition ?? rec.formula ?? "", rec.facet ?? ""].filter(Boolean).join(" ") : "";
  }
  ...
  const unusedSurfaces = [...surfaces.values()].filter((r) => r.uses === 0).map((r) => `${r.label}${r.viaContainer ? ` (${r.viaContainer} via a container)` : " (unused)"}`);
  const checks = [
    {
      label: "Surfaces reached only through a coarser one",
      detail: "The claim named the catalyst, not the site, and the parts list is what connects the two. Without it these bands would be invisible to any site query.",
      hits: [...surfaces.values()].filter((r) => (r.viaContainer ?? 0) > 0).sort((a, b) => (b.viaContainer ?? 0) - (a.viaContainer ?? 0)).map((r) => `${r.label}: ${r.viaContainer} claims`)
    },
    {
    ...
    },
    {
      ...
      detail: "A free molecule sits on nothing. Naming the sample is fine, since the gas was measured over it, but a site-level key is either a leftover from before the band was known to be gas phase or a claim that belongs on an adsorbed band instead.",
      hits: gasWithSite
    },
    {
    ...
    },
    {
      ...
      detail: "R/P/Q branches are one transition of one species, so siblings must agree on species and phase. Rotational structure also only exists for a freely rotating gas molecule, which makes an adsorbed branch sibling a contradiction rather than a typo.",
      hits: branchDisagreements
    },
    {
      label: "Topologies the molecule does not declare",
      detail: "The band names a binding geometry that its species\u2019 molecule has no Topology for, so the vibration-modes page cannot draw it. A to-do for vibrations.jsonc, not an error in the band.",
      hits: undeclaredTopology
    },
    {
      label: "Bands with no phase",
      detail: "Deliberate where the band covers both the free molecule and its adsorbed form, which is the case for several methanol fundamentals. Worth a look anywhere else.",
      hits: noPhase
    },
    {
      label: "Surfaces never named directly",
      detail: "Present in the table but never written into a claim. Fine for an entry that only exists inside another one\u2019s parts list.",
      hits: unusedSurfaces
    },
    {
      label: "Claims with several sites and several wavenumbers",
      detail: "Still ambiguous: nothing says which peak was seen on which site. Splitting the claim into one row per site resolves it.",
      hits: ambiguousClaim
    },
    {
